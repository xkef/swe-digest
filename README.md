...
injection eventually succeeds and bounds what it can reach.
...
- **The agent cannot rewrite its own gate.** Gate code sits outside the path
  allowlist and runs from a fresh checkout of `main`. The attacker model and
  the control for each attack path are in
  [`routine/threat-model.md`](routine/threat-model.md), and the test suite
  replays prompt-injection attacks against the gates.
- **Its memory is public and bounded.** Context between runs is plain
  markdown under a schema gate that enforces size bounds and dated entries,
  so memory decays unless re-verified instead of growing into a second,
...
- **Blocked fetches do not blank a day.** A background workflow accumulates
  each source into dated snapshots, so a run whose live fetch is refused
  still has the day's coverage. Details:
...
- **Anyone can suggest a story.** Outside issues pass through deterministic
  ...
  suggestion, publishes it, and closes the issue with a link.
...
- [`routine/`](routine/README.md): the owner-gated instruction set, the
  watchlist, the threat model, and the schedule that runs them.
