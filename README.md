...
1. Collect. Hacker News, Reddit, release feeds, papers, books, videos, and a
   watchlist. Plain Python. Responses are committed as snapshots.
...
- [src/swe_digest/](src/swe_digest/): the Python package. Layer boundaries are
  enforced by import-linter in CI.
- [config/](config/), [prompts/](prompts/): human owned. A run may propose
  config changes through a pull request. It may not touch prompts.
- [data/](data/): everything the agent writes, including digests, run logs,
  ...
  thin or rate-limited still read the day's coverage.
- [site/](site/): Zola source, hand written. Day pages are generated at build
  time, so nothing under it is in the publish allowlist.
- .github/workflows/: schedules, the two-job publish pipeline, issue triage.

[SECURITY.md](SECURITY.md) has the threat model and disclosure policy.
