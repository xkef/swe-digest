...
Separate from the fetchers, so a caller that wants only the queries, such as the
run log, does not import the fetch layer to get them.
"""
...
def load_watchlist() -> dict[str, Any]:
    """Returns the parsed watchlist, for a caller to pluck its own table from."""
    with paths.watchlist_file().open("rb") as handle:
        return tomllib.load(handle)


def entries(table: str, key: str) -> list[str]:
    """Returns one list out of the watchlist, or exits naming what is missing.

    A source with nothing configured has nothing to fetch, and saying so is the
    only useful thing it can do.
    """
...
def pairs(table: str, key: str, *, valid: Callable[[str], bool]) -> list[tuple[str, str]]:
    """Splits ``"one|other"`` watchlist entries into (label, value).

    The half that satisfies ``valid`` is the value and the other is the label,
    so both orders the watchlist uses need no flag and no edit to the file. An
    entry with no valid half is dropped.
    """
    out = []
