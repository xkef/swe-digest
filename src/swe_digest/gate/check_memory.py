...
entries, and `Last seen` dates on every tracked entity. Content screening
...
ENTITY_BULLET = re.compile(r"^- (?P<text>.+)$", re.MULTILINE)
...
def check_entities(path: Path, text: str, today: date) -> list[str]:
    errors = []
    stale: list[str] = []
    for match in ENTITY_BULLET.finditer(strip_fences(text)):
        ...
        if not seen:
            errors.append(f"{path}: entity entry lacks 'Last seen YYYY-MM-DD': {bullet[:60]!r}")
            continue
        seen_date = parse_iso(seen.group("date"))
        if seen_date is None:
            errors.append(f"{path}: invalid Last seen date: {bullet[:60]!r}")
        elif (today - seen_date).days > config.MEMORY_ENTITY_STALE_DAYS:
            stale.append(bullet[:60])
    if stale:
        print(
            f"warn: {path}: {len(stale)} entities not seen for"
            f" {config.MEMORY_ENTITY_STALE_DAYS}+ days; prune or refresh:",
            file=sys.stderr,
        )
        for entry in stale:
