...
Already run: the stores under ``data/memory/`` are the source of truth and the
...
that rewrote published content:
    ...
    git checkout <pre-migration-sha> -- data/memory
...
from swe_digest.domain.records import STORES, Followup, Note, Record
...
def bullets(text: str) -> list[str]:
    ...
    wrapped entry reads as one unit.

    Here rather than in the memory gate because the markdown these parse has not
    existed since the stores became typed. This module is their only caller.
    """
    out: list[str] = []
    in_bullet = False
...
def migrate(root: Path | None = None, *, write: bool = True) -> int:
    base = paths.MEMORY_STORE.dir(root)
    failures = 0
    for name in STORES:
        ...
        if lost:
            ...
            continue
        if write:
            memory_store.save(name, records, root)
