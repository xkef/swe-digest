...
def repo_tree(tmp_path: Path) -> Path:
    ...
    Built from the path families rather than from literals, so a fixture tree
    cannot drift from the layout the gates actually walk.
    """
    ...
    # An empty store rather than no file: the gate walks what exists, and a
    # tree with no store at all is not a shape the repository ever has.
...
def write_run_log(root: Path, date: str, record: dict[str, Any]) -> Path:
    """A run log the way the pipeline writes one, in the canonical form the
    content gate checks."""
...
def git(cwd: Path, *args: str) -> str:
    proc = subprocess.run(
        [
            ...
            # A developer with commit signing configured globally would otherwise
            # have every fixture repo fail on a signing agent these tests neither
            # need nor should reach.
            "-c",
...
def git_repo(repo_tree: Path) -> Path:
    ...
    # Written into the fixture repo's own config, so operations that do not go
    # through the helper above — `git am` inside the publish gate, for one —
    # also never reach a signing agent.
...
def at_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point the whole package at a fixture tree.

    Every path family resolves through ``paths.ROOT`` when it is asked, so one
    patch moves all of them together — rather than a module-level constant per
    module, where a test that missed one silently read the real repository.
    """
...
def _never_write_the_real_repo(
    ...
    """Point ``paths.ROOT`` at a scratch tree for every test that has not
