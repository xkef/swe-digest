...
ISSUES = f"repos/{settings.REPO}/issues?state=open&labels=feedback&per_page=100"
...
def issue(number: int, kind: str, login: str = settings.OWNER, **extra: Any) -> dict[str, Any]:
...
def test_a_kind_becomes_the_record_its_mapping_names(tmp_path: Path) -> None:
    ...
    records = memory_store.load("followups", tmp_path)
    ...
    assert closes == [
        {
            "number": 11,
            "comment": feedback.CLOSE_COMMENT.format(kind="not interesting", site=settings.SITE),
        }
    ]
...
def test_a_wrong_source_lands_in_source_reliability(tmp_path: Path) -> None:
    ...
    assert len(memory_store.load("source-reliability", tmp_path)) == 1
...
def test_weekly_only_kinds_write_no_memory_but_still_close(kind: str, tmp_path: Path) -> None:
    ...
    assert [
        len(memory_store.load(name, tmp_path)) for name in ("followups", "source-reliability")
    ] == [
        0,
        0,
    ]
...
def test_the_close_comment_satisfies_the_publish_gate() -> None:
    ...
    publish.check_comment(1, feedback.CLOSE_COMMENT.format(kind="other", site=settings.SITE))
...
def test_details_stay_out_of_the_subject(tmp_path: Path) -> None:
    ...
    assert len(record.notes) <= settings.MEMORY_MAX_LINE_CHARS
