...
`run_log` writes `data/runs/YYYY-MM-DD.yaml` after the digest is
...
step may write `data/memory/` — the write guard grants the write step the
...
Run logs are the durable evidence store. `data/snapshots/hn/` snapshots are
pruned to
...
into `data/runs/weekly/YYYY-MM-DD.yaml`. The script owns the marker's
...
- A personalization proposal needs a technology, topic, or org recurring across
  ...
  follow.
- The proposed diff touches only `config/`: the watchlist, the tunables,
