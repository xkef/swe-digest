...
def interest_signal(gh: GitGh) -> dict:
    ...
    and absent from the watchlist and the profile. Computed rather than asked
    for, because no step has the shell this used to need.
    ...
    **Aggregate only.** Repository names, starred repos, and follows are
    ...
    try:
        for repo in gh.gh_json(f"users/{config.OWNER}/repos?per_page=100"):
            tally("languages", [repo.get("language")])
            tally("topics", repo.get("topics"))
        for repo in gh.gh_json(f"users/{config.OWNER}/starred?per_page=100"):
            tally("languages", [repo.get("language")])
            tally("topics", repo.get("topics"))
            tally("orgs", [(repo.get("owner") or {}).get("login")])
        for account in gh.gh_json(f"users/{config.OWNER}/following?per_page=100"):
