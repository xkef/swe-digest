...
Memory used to be markdown the agent edited by hand, which meant the schema
was a convention: dates drifted, `Status: open` markers went missing, and the
gate could only notice at publish time. These records make the schema the type,
and every field the model must not choose — identity, dates, status — is set by
``store.py`` rather than supplied by a caller.
...
class StoreSpec:
    ...
    record: type[Record]
    title: str
    # Bound key in swe_digest.config, enforced by the store on write.
    max_entries: str
