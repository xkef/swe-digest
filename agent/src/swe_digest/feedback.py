...
Feedback used to be a paragraph of prompt: the model listed `feedback` issues,
decided what each one meant, wrote memory, and asked for the close. None of
that needs a model. The issue form is structured — `Kind` is a dropdown, with
`Story`, `Digest date`, `Topic`, and `Details` beside it — so the mapping from
a kind to a memory record is a lookup, and a lookup is testable, cheap, and
cannot be argued into a different answer by the issue text.
...
def owner_issues(gh: GitGh, label: str = "feedback") -> list[dict[str, Any]]:
    ...
    """
    path = f"repos/{config.REPO}/issues?state=open&labels={label}&per_page=100"
    return [
        issue
        for issue in gh.gh_json(path)
        if "pull_request" not in issue and (issue.get("user") or {}).get("login") == config.OWNER
    ]
...
def record_for(item: Feedback) -> dict[str, str]:
    ...
    return {
        ...
        "notes": context[: config.MEMORY_MAX_LINE_CHARS],
    }
...
def process(gh: GitGh | None = None, root: Any = None) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for issue in owner_issues(gh):
        ...
        if name == "source-reliability":
            store.add(name, root, **note_for(item))
        elif name:
            store.add(name, root, **record_for(item))

        closes.append(
            {
                "number": item.number,
                "comment": CLOSE_COMMENT.format(kind=item.kind, site=config.SITE),
            }
        )
