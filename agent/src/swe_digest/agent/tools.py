...
MAX_OUTPUT_CHARS = config.AGENT_TOOL_OUTPUT_MAX_CHARS
...
def _memory_handler(spec: catalog.AgentTool) -> Handler:
    ...
    Edit on ``agent/memory/``. Identity and dates are assigned by the store, so
    ...
    async def handler(args: dict[str, Any]) -> Result:
        ...
        try:
            match action:
                ...
                case "add":
                    record = await asyncio.to_thread(store.add, name, None, **values)
                    payload = {"store": name, "added": json.loads(record.to_json())}
                case "touch":
                    record = await asyncio.to_thread(store.touch, name, values["id"], None)
                    payload = {"store": name, "touched": json.loads(record.to_json())}
                case _:
                    await asyncio.to_thread(store.close, name, values["id"], None)
...
async def _guidance_handler(args: dict[str, Any]) -> Result:
    ...
    """
    topic = args["topic"]
    path = ROOT / "agent" / "prompts" / "sources" / f"{topic}.md"
    try:
