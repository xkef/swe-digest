...
from swe_digest.digest import canonical
...
from swe_digest.paths import MEMORY_REL, ROOT
...
def check_stories(path: Path, text: str) -> list[str]:
    ...
    check_categories = path.parent.name >= CATEGORY_SINCE
    slugs: dict[str, str] = {}
...
def check_run_logs(root: Path) -> list[str]:
    ...
    errors: list[str] = []
    for path in sorted((root / MEMORY_REL / "runs").glob("*.yaml")):
        try:
...
def check_repo_links(root: Path) -> list[str]:
    ...
    """
    pattern = re.compile(rf"https://github\.com/{re.escape(config.REPO)}/blob/main/([^)\s\"'<>#]+)")
    errors: list[str] = []
    # The digests only. Story pages under site/content/stories/ are generated
    # from them, so checking the source covers both and cannot trip over a
    # stale artifact from an earlier build.
    for path in sorted((root / "site" / "content" / "digests").rglob("*.md")):
