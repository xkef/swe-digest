...
# The memory stores an unattended run may write. It writes them through
# memory.store, never with an editor: the model has no Write/Edit grant on
# agent/memory/, so the schema and the bounds cannot drift. These paths stay in
# the allowlist because the commit still has to carry the changed stores.
...
ALLOWED_PATHS = [
    ...
    re.compile(r"^agent/memory/runs/\d{4}-\d{2}-\d{2}\.yaml$"),
    re.compile(r"^agent/memory/runs/weekly/\d{4}-\d{2}-\d{2}\.yaml$"),
    re.compile(rf"^agent/memory/({'|'.join(MEMORY_FILES)})\.yaml$"),
]
# Proposable through the owner-approved improvement PR, never written by a run.
# agent/prompts/ is deliberately absent: the prompts are maintainer-only, so a
# run cannot propose edits to its own instructions.
IMPROVEMENT_FILES = {
    "agent/config/config.toml",
    "agent/config/watchlist.toml",
    "agent/config/profile.md",
}
...
def writable_paths(date: str, root: Path | None = None) -> list[str]:
    ...
    digest, when it exists.

    The memory stores are not here. ``memory.store`` writes them in exactly one
    valid form, which the content gate checks, so there is nothing for a
    formatter to decide."""
    root = root or ROOT
    digest = f"site/content/digests/{date}/index.md"
    return [digest] if (root / digest).exists() else []
