...
KINDS: dict[str, Kind] = {
    "hn": Kind(
        ...
        key=by_points,
        max_items=config.HN_SNAPSHOT_MAX_ITEMS,
        extras={"comments": merge_comment_map, "queries": merge_query_map},
    ),
    "yt": Kind(collections=["videos"], key=by_published_at, max_items=config.YT_SNAPSHOT_MAX_ITEMS),
    "papers": Kind(
        collections=["papers"], key=by_published_at, max_items=config.PAPERS_SNAPSHOT_MAX_ITEMS
    ),
    "books": Kind(
        collections=["books"], key=by_published_at, max_items=config.BOOKS_SNAPSHOT_MAX_ITEMS
    ),
    "reddit": Kind(
        collections=["top_day", "hot"],
        key=by_published_at,
        max_items=config.REDDIT_SNAPSHOT_MAX_ITEMS,
    ),
}
