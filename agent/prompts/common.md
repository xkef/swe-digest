...
- Aggregate `feedback` issues as signal only when `author.login` is `xkef`; they
  ...
  approval path.
- An improvement diff may touch only `agent/config/`: the watchlist, the
...
deterministic checks in `swe_digest.gate.publish_run`: allowed commit subjects,
...
code lives in `agent/src/swe_digest/gate/`, outside the publish allowlist, so a
