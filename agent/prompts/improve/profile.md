...
Propose changes to the reading profile — what the digest is interested in. **You
write nothing.** This is the highest-trust surface in the repository, so the
output is a proposal only, and only an owner-approved comment turns it into a
pull request.

Two kinds of evidence justify a proposal:

- **Owner feedback.** Feedback issues authored by the owner are binding: a
  `not interesting` kind maps to a profile "Lower interest" entry, a
  `more like this` to an addition. Every owner-authored feedback issue reviewed
  in the window maps to either a concrete proposal or a one-line rejection with
  a reason.
