...
def test_staged_changes_become_one_verified_commit(
    ...
    monkeypatch.chdir(git_repo)
    monkeypatch.setenv("GITHUB_REPOSITORY", "o/r")
    (git_repo / "snapshots" / "hn").mkdir(parents=True)
    (git_repo / "snapshots" / "hn" / "2026-07-04.json").write_text("{}\n")
    (git_repo / MEMORY_REL / "followups.md").unlink()
    git(git_repo, "add", "-A")
    ...
    assert [a["path"] for a in additions] == ["snapshots/hn/2026-07-04.json"]
    assert deletions == [{"path": "agent/memory/followups.md"}]
