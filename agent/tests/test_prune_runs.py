...
from swe_digest.digest import prune, runs
...
def run_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    directory.mkdir()
    monkeypatch.setattr(prune, "RUNS_DIR", directory)
...
def test_compaction_never_unlinks_a_log(run_dir: Path) -> None:
    ...
    old = write(run_dir, days_ago=config.MEMORY_RUN_DETAIL_DAYS + 30)
...
def test_old_logs_lose_ids_but_keep_counts(run_dir: Path) -> None:
    ...
    path = write(run_dir, days_ago=config.MEMORY_RUN_DETAIL_DAYS + 1)
...
def test_the_boundary_day_is_kept(run_dir: Path) -> None:
    path = write(run_dir, days_ago=config.MEMORY_RUN_DETAIL_DAYS)
