...
def test_cap_keeps_the_highest_scoring_stories(tmp_path: Path, monkeypatch: Any) -> None:
    ...
    import swe_digest.snapshot.merge as merge

    spec = merge.KINDS["hn"]
    monkeypatch.setitem(merge.KINDS, "hn", replace(spec, max_items=3))
...
def test_cap_bounds_the_first_write_too(tmp_path: Path, monkeypatch: Any) -> None:
    ...
    spec = merge.KINDS["hn"]
    monkeypatch.setitem(merge.KINDS, "hn", replace(spec, max_items=2))
...
def test_every_kind_declares_a_cap_above_observed_volume() -> None:
    ...
    from swe_digest.snapshot.merge import KINDS
