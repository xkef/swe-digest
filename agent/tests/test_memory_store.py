...
The properties worth pinning are the ones that make the store better than the
...
from swe_digest.memory.records import Followup, Note


def test_add_assigns_identity_and_dates(tmp_path: Path) -> None:
    """The caller supplies content; the store supplies everything else.

    This is the whole point: a model cannot forget a date or invent an id.
    """
    record = store.add("followups", tmp_path, subject="Zig 0.15", watch_for="the release")
...
def test_records_round_trip_through_the_file(tmp_path: Path) -> None:
    store.add("entities", tmp_path, subject="Neovim", note="editor", group="Dev tools")
    store.add("entities", tmp_path, subject="Zig", note="language", group="Languages")

    loaded = store.load("entities", tmp_path)
...
def test_lines_are_stable_so_git_shows_no_spurious_diff(tmp_path: Path) -> None:
    ...
    store.add("entities", tmp_path, subject="Neovim", note="editor")
    before = store.path_for("entities", tmp_path).read_bytes()

    store.save("entities", store.load("entities", tmp_path), tmp_path)

    assert store.path_for("entities", tmp_path).read_bytes() == before


def test_ids_stay_unique_after_a_close(tmp_path: Path) -> None:
    """Reusing a freed id would make history ambiguous."""
    store.add("followups", tmp_path, subject="one")
    second = store.add("followups", tmp_path, subject="two")
    store.close("followups", "f-0001", tmp_path)

    third = store.add("followups", tmp_path, subject="three")
...
def test_entry_bound_fails_the_write_that_breaks_it(tmp_path: Path) -> None:
    ...
    for n in range(config.MEMORY_MAX_OPEN_FOLLOWUPS):
        store.add("followups", tmp_path, subject=f"item {n}")

    with pytest.raises(store.StoreError, match="exceeds the bound"):
        store.add("followups", tmp_path, subject="one too many")
...
def test_byte_bound_is_enforced_on_write(tmp_path: Path) -> None:
    ...
    fat = "x" * (config.MEMORY_MAX_FILE_BYTES // 4)
    with pytest.raises(store.StoreError, match="bytes exceeds"):
        for n in range(5):
            store.add("entities", tmp_path, subject=f"e{n}", note=fat)
...
def test_a_failed_write_leaves_the_previous_store_intact(tmp_path: Path) -> None:
    ...
    store.add("entities", tmp_path, subject="keep me", note="small")
    before = store.path_for("entities", tmp_path).read_text()

    with pytest.raises(store.StoreError):
        store.add("entities", tmp_path, subject="huge", note="x" * config.MEMORY_MAX_FILE_BYTES)

    assert store.path_for("entities", tmp_path).read_text() == before


def test_a_malformed_store_is_loud(tmp_path: Path) -> None:
    """A partial read would silently lose a tracked fact."""
    store.add("entities", tmp_path, subject="ok", note="fine")
    file = store.path_for("entities", tmp_path)
    file.write_text(file.read_text() + "  - [unclosed\n", encoding="utf-8")

    with pytest.raises(store.StoreError, match="malformed store"):
        store.load("entities", tmp_path)
...
def test_a_store_that_is_not_an_array_is_loud(tmp_path: Path) -> None:
    ...
    with pytest.raises(store.StoreError, match="array of records"):
        store.load("entities", tmp_path)


def test_unknown_fields_are_dropped_not_rejected(tmp_path: Path) -> None:
    """A store written by a newer schema still loads; the gate decides validity."""
    store.add("entities", tmp_path, subject="ok", note="fine")
    file = store.path_for("entities", tmp_path)
    record = serial.load(file.read_text())[0] | {"from_the_future": 1}
    file.write_text(serial.dump([record]), encoding="utf-8")

    assert store.load("entities", tmp_path)[0].id == "e-0001"
...
def test_touch_redates_without_restating_content(
    ...
    touched = store.touch("entities", "e-0001", tmp_path)
...
def test_close_and_update_reject_an_unknown_id(tmp_path: Path) -> None:
    ...
    with pytest.raises(store.StoreError, match="no record"):
        store.close("entities", "e-9999", tmp_path)
    with pytest.raises(store.StoreError, match="no record"):
        store.update("entities", "e-9999", tmp_path, note="x")


def aged(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, when: str, **values: str) -> None:
    """Add a record as of a past date.

    The store owns `last_seen` — passing it to `update` is overridden on
    purpose — so ageing a record means moving the clock, the same way the
    fetchers inject one.
    """
    monkeypatch.setattr(store, "today", lambda: when)
    store.add("entities", tmp_path, **values)
    monkeypatch.undo()
...
def test_prune_drops_only_what_is_older_than_the_age(
    ...
    store.add("entities", tmp_path, subject="fresh", note="new")
    aged(tmp_path, monkeypatch, "2020-01-01", subject="stale", note="old")

    dropped = store.prune("entities", 30, tmp_path)

    assert [r.id for r in dropped] == ["e-0002"]
    assert [r.id for r in store.load("entities", tmp_path)] == ["e-0001"]
...
def test_query_filters_by_age_and_text(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert [r.id for r in store.query("entities", tmp_path, contains="neovim")] == ["e-0001"]
    assert [r.id for r in store.query("entities", tmp_path, older_than_days=30)] == ["e-0002"]


def test_unknown_store_names_are_refused(tmp_path: Path) -> None:
    with pytest.raises(store.StoreError, match="unknown store"):
        store.add("not-a-store", tmp_path, subject="x")
