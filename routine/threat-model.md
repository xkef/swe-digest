...
- GitHub issues and comments (public; the feedback links on the site invite
  them).
- Previously committed memory (`memory/*.md`) and snapshots (`snapshots/*/*.json`),
...
- every added or modified path in every commit matched against the publish
  allowlist (`site/content/digests/`, `memory/runs/`, and the four writable
...
- improvement PRs require an `OWNER` approval comment, apply only the diff
  from the issue body, and may touch only `routine/config.toml`,
  `routine/watchlist.toml`, `memory/profile.md`, `routine/routine.md`, and
  `CLAUDE.md`.

GitHub additionally rejects any `GITHUB_TOKEN` push that modifies
`.github/workflows/`. The validator itself lives in `tool/src/swe_digest/gate/`,
