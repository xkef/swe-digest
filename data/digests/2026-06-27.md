...
- **Blurb:** OpenAI previewed three-tier GPT-5.6: Sol flagship, Terra halved cost, Luna budget; adds max and ultra reasoning modes.
...
- **Blurb:** Lutnick cleared Mythos 5 for over 100 US institutions; OpenAI shifts to per-customer vetting of GPT-5.6.
...
- **Blurb:** AWS announced Lambda MicroVMs, Firecracker serverless sandboxes with 16 vCPUs, 32GB memory, suspend-resume for untrusted code.
...
- **Blurb:** Linux Foundation launched Akrites with 18 industry members coordinating open-source vulnerability disclosure and maintainer support.
...
- **Blurb:** DeepSeek published MIT-licensed DeepSpec for training speculative-decoding drafts; DSpark provides lossless speedup.
...
- **Blurb:** Doubleword analysis shows open-weights to closed-source gap depends on benchmark; coding narrowed fastest, average gap five months.
...
- **Blurb:** Rehman et al. published ICML spotlight ArBG, replacing normalizing flows with transformers for molecular equilibria sampling.
...
- **Blurb:** Nx announced Polygraph, connecting repositories in one graph; agents read and write across repos without moving code.
...
- **Blurb:** Show HN posted workweave/router, a model router selecting models per prompt in 50ms with claimed 40-70% cost reduction.
...
- **Blurb:** Matt Mastracci documented a targeted attack via fake VC offer, leading to TypeScript patch with multi-stage RAT delivery.
...
- **Blurb:** tmux 3.7 added floating panes above tiled layouts, line numbers in copy mode, and tightened read-only permission checks.
...
- **Blurb:** GCC 14.4 released as bug-fix point update fixing regressions in the 14 series for teams using GCC 14.
...
- **Blurb:** Mac Chaffee revisited AOL's 19-hour 1996 outage, arguing SRE reliability cases should center human impact beyond cost.
...
- **Blurb:** Voytek Pitula published a living Fintech Engineering Handbook covering money representation, bookkeeping, flows, and testing.
...
- **Blurb:** A satirical incident report styled as CVE and postmortem reached HN with 500 points, parodying code review culture.
...
- **Blurb:** The GPT-5.6 preview thread debated naming, generational skipping, and pricing parity over 500 comments.
...
- **Blurb:** Andrej Karpathy called Claude Tag the third major LLM UI redesign; reception was mixed on genuine workflow impact.
