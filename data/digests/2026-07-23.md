...
- **Blurb:** Qualys disclosed RefluXFS, a race in XFS copy-on-write escalating to root on default RHEL, Oracle, and Amazon Linux, amid a 432-CVE kernel flood.
...
- **Blurb:** White House accused Moonshot of distilling Fable to build Kimi K3, escalating US-China AI tensions into policy and possible-sanctions tracks.
...
- **Blurb:** Pillar Security disclosed sandbox escapes in Cursor, Codex, and Gemini CLI via the Docker socket, allowing writes to files outside the workspace.
...
- **Blurb:** CISA added a Check Point SmartConsole auth bypass giving admin access and a fourth SharePoint RCE to KEV with federal remediation due soon.
...
- **Blurb:** PyPI now rejects new files to releases older than 14 days to prevent poisoning of long-stable pinned releases through compromised credentials.
...
- **Blurb:** A coalition of 200 startups urged Trump not to restrict Chinese open-weight AI like Kimi and Qwen, saying many builders depend on them.
...
- **Blurb:** OpenAI introduced Presence for building support agents with guardrails and escalation, using Codex to review and approve behavior changes.
...
- **Blurb:** An essay questions whether AI labs game informal public benchmarks like drawing pelicans, warning improvements may reflect targeted training.
...
- **Blurb:** GigaToken, a Rust tokenizer, reports 989x faster throughput than Hugging Face and 681x over tiktoken on GPT-2 BPE with SIMD pretokenization.
...
- **Blurb:** A practitioner review graded 36 MCP servers on agent usability, finding a third scored poorly on descriptions, bloated responses, and errors.
...
- **Blurb:** A developer found staged git pre-commit hook malware in a fake take-home project delivered via social-engineered fake recruiting outreach.
...
- **Blurb:** Kata Containers 4.0.0 shipped a Rust runtime replacing Go, becoming the new default for VM-isolated container workloads in Kubernetes.
...
- **Blurb:** Codeberg amended its terms to ban cryptocurrency and blockchain projects, a second content-policy narrowing from the forge in recent weeks.
...
- **Blurb:** Greg Kroah-Hartman said Rust makes kernel coding fun and draws new contributors, signaling the direction of future kernel contributor onboarding.
...
- **Blurb:** Cruller forked Bun's last Zig runtime and updated it to Zig 0.16.0, keeping production features while stripping dev tooling for a smaller binary.
...
- **Blurb:** Safari Technology Preview 248 shipped with WebKit fixes and platform features across CSS, JavaScript, rendering, and web APIs.
...
- **Blurb:** Hatchet published a Postgres guide covering schema design, connection management, index pitfalls, and recurring failure modes at scale.
...
- **Blurb:** Mitchell Hashimoto argues working engineers should understand SIMD, explaining where single-core performance now lives with Zig examples.
...
- **Blurb:** A writeup explains git's argument injection defense via `--end-of-options`, finding only Go's toolchain consistently applies it across managers.
...
- **Blurb:** Nikkei Asia reports Alphabet, Amazon, Meta, Microsoft, and Oracle carry $1.65 trillion in off-balance-sheet AI-infrastructure debt through SPVs.
...
- **Blurb:** Bento, a Show HN, presents an entire slide deck (editing, viewing, collaboration) in one self-contained HTML file, topping the HN front page.
...
- **Blurb:** An essay argues valuing used GPU clusters is hard given fast depreciation, uncertain useful life, and thin markets, affecting capex assumptions.
...
- **Blurb:** Reddit now requires login to browse old.reddit.com, ending anonymous access to the legacy interface to stop abusive scraping.
...
- **Blurb:** r/cursor reports strong satisfaction with Grok 4.5 on cost and coding quality, continuing adoption sentiment for lower-cost models in agents.
