...
- **Blurb:** CISA added CVE-2026-45659, a deserialization RCE in on-premises SharePoint, to the KEV catalog with a federal deadline of 2026-07-04.
...
- **Blurb:** Anthropic redeployed Fable 5 after export controls lifted, adding a classifier blocking the reported jailbreak in over 99 percent of cases.
...
- **Blurb:** Google enforces developer verification on certified Android devices starting 2026-09-30, blocking unverified app installs in four countries.
...
- **Blurb:** Cloudflare announced Monetization Gateway for charging access via x402, an open protocol for HTTP-based micropayments in stablecoins.
...
- **Blurb:** VoidZero released Vite+ beta, an MIT-licensed toolchain bundling Vite, Vitest, Rolldown, Oxlint, and Oxfmt with monorepo support.
...
- **Blurb:** Jujutsu 0.43.0 adds `jj run` for batch command execution across changes, removing Git-symbol resolution and deprecated revset functions.
...
- **Blurb:** A preprint proposes SynLaD, a latent-diffusion model for de novo molecules conditioned on pharmacophore and synthesis-route feasibility.
...
- **Blurb:** Z.ai released ZCode, a first-party coding-agent harness for GLM-5.2 with 1M-token context and chat-app task triggers for WeChat and Feishu.
...
- **Blurb:** GitHub made Moonshot AI's open-weight Kimi K2.7 Code model generally available in Copilot across VS Code, Visual Studio, and JetBrains.
...
- **Blurb:** Senior SWE-Bench measures coding agents on 50 investigation-heavy tasks, finding Claude Opus 4.8 solves 24 percent of senior-level work.
...
- **Blurb:** A rewritten FFmpeg native AAC encoder surfaces for review, scoring above Apple Core Audio in constant-bitrate encoding-quality tests.
...
- **Blurb:** Asahi Linux 7.1 adds M3 audio output and hardware H.264 decoding to 10-bit 4K via a new V4L2 driver with custom AVD firmware.
...
- **Blurb:** Prometheus 3.13.0 LTS fixes UI cross-site scripting and stops forwarding credentials on cross-host redirects in scraping and remote write.
...
- **Blurb:** Grafana 13.1.0 adds colorblind-safe series patterns and Mimir Alertmanager auto-sync for dashboard and alerting operability.
...
- **Blurb:** Probelab optimized IPFS publishing by returning after 15 of 20 provider announcements, cutting perceived latency an order of magnitude.
...
- **Blurb:** The July 2026 Ask HN hiring threads are live with the Who is hiring thread at 164 points and Who wants to be hired thread at 108 points.
...
- **Blurb:** A graphics programming thread on Hacker News collected practitioner recommendations on math fundamentals, rendering, and project paths.
