...
- **Blurb:** AWS Middle East Bahrain region is offline for months after conflict damage, forcing customers to migrate workloads to other regions.
...
- **Blurb:** Anthropic released Claude Opus 5 at Opus 4.8 pricing with state-of-the-art coding eval scores in API, Claude.ai, and Claude Code.
...
- **Blurb:** OpenAI and Anthropic lobbied for Chinese open-weight restrictions while Nvidia, Meta, Microsoft, and others urged against prohibitions.
...
- **Blurb:** A post argues fully automated coding pipelines degrade maintainability, proposing human design phases before agent implementation.
...
- **Blurb:** Azure West US experienced connectivity failures from a network-infrastructure issue, resolved by rolling back a recent network change.
...
- **Blurb:** Black Forest Labs announced FLUX 3 for image, video, and audio with text-to-video and video editing support, weights pending.
...
- **Blurb:** Hanwha firmware leaked a GitHub admin token in the login UI by exporting CI environment variables into the frontend bundle via Vite.
...
- **Blurb:** India ordered GitHub to remove Bitchat decentralized messenger repositories, arguing the app frustrates lawful interception efforts.
...
- **Blurb:** JEP 540 defines an incubating JSON API for the JDK to remove third-party dependencies like Jackson or Gson for basic parsing.
...
- **Blurb:** Deno 2.9.4 upgrades V8 to 150.2.0, adds Node-compatibility features, and tightens FFI permission requirements for native window handles.
...
- **Blurb:** Buz is a work-in-progress fork of Bun from before the Zig-to-Rust rewrite, targeting 1.4.0 feature parity with sub-one-second incremental builds.
...
- **Blurb:** Stripe is in talks to acquire OpenRouter, an AI model marketplace serving five million developers with unified routing across hundreds of models.
...
- **Blurb:** A Tell HN reports Namecheap gave account control to an unverified third party, adding to reports of declining account handling at registrars.
...
- **Blurb:** Cursor users report that selecting Grok 4.5 at high reasoning effort routes through metered API usage rather than the included plan allotment.
...
- **Blurb:** Armin Ronacher criticized Codeberg's AI-code ban as unenforceable and too discretionary, arguing for stricter rules or abuse-focused moderation.
