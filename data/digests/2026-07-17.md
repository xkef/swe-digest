...
- **Blurb:** Three unauthenticated remote-code flaws added to KEV under active exploitation: SharePoint and FortiSandbox with federal remediation deadline.
...
- **Blurb:** AWS Cost Explorer displayed inaccurate billing estimates, with accounts seeing millions in charges on zero usage from a unit-pricing defect.
...
- **Blurb:** Moonshot published Kimi K3 as a 2.8T Mixture-of-Experts model with 1M-token context and $0.30/$3.00/$15.00 per 1M tokens, weights due July 27.
...
- **Blurb:** Turso plans to add Postgres compatibility to its Rust engine, reusing the same bytecode core across SQLite and Postgres frontends.
...
- **Blurb:** A German consortium released Soofi S 30B, a hybrid Mamba and Transformer Mixture-of-Experts model trained on Deutsche Telekom's Munich cloud.
...
- **Blurb:** EuroPython 2026 runs through July 19 with conference talks and sprints on CPython, typing, packaging, and the scientific Python ecosystem.
...
- **Blurb:** Google renamed NotebookLM to Gemini Notebook with Gemini integration, code execution for Pro users, and Search integration.
...
- **Blurb:** Ring-Zero reports applying zero-reinforcement learning to a 1-trillion-parameter model across seven mathematical benchmarks.
...
- **Blurb:** LM Studio released Bionic, a desktop agent for running open models locally on coding tasks with optional cloud fallback.
...
- **Blurb:** A Pydantic article argues LLM-assisted programming replaces coding work with supervisory load, causing reviewer fatigue and rubber-stamping.
...
- **Blurb:** GitHub REST API degraded for 90 minutes with 39% of requests failing, caused by infrastructure change marking API backends as unhealthy.
...
- **Blurb:** Solod is a strict Go subset that compiles to C11 without runtime, garbage collection, or hidden allocations, with native C interop.
...
- **Blurb:** Homescale describes building writable database branches from immutable snapshots using Kubernetes, Ceph, and copy-on-write cloning.
...
- **Blurb:** LLM output is detectable via scikit-learn SVM from statistical patterns, though round-trip translation and other evasions degrade detection.
...
- **Blurb:** Apple sent legal-preservation letters to about 40 former employees now at OpenAI, signaling the case extends beyond the two named defendants.
...
- **Blurb:** An essay accepts LLM criticisms about environmental cost, copyright, and geopolitics, then argues the author still uses LLMs heavily anyway.
