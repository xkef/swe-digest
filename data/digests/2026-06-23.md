...
- **Blurb:** OpenAI announced Daybreak with updated GPT-5.5-Cyber and Patch the Planet, which landed 64 pull requests in Python, Go, and cURL in week one.
...
- **Blurb:** OpenAI fixed a Codex CLI bug that wrote 37 TB of logs in three weeks; the fix in rust-v0.142.0 resolves an SSD endurance risk for daily users.
...
- **Blurb:** Mitchell Hashimoto pledged $400,000 to the Zig Software Foundation, bringing his total to $700,000 and endorsing its no-LLM contribution stance.
...
- **Blurb:** MIT researchers frame prompt injection as role confusion: stripping attack text's style dropped success from 61% to 10%, suggesting a defense.
...
- **Blurb:** ICML 2026 starts in 13 days at 2026-07-06, hosting primary-venue machine learning and security results this digest tracks.
...
- **Blurb:** EuroPython 2026 runs 2026-07-13 through 2026-07-19, showcasing CPython, packaging, and language-ecosystem changes.
...
- **Blurb:** Mistral released OCR 4 with structured output including bounding boxes and confidence scores across 170 languages, priced at $4 per 1,000 pages.
...
- **Blurb:** Unsloth documented local GLM-5.2 deployment, reporting that a 2-bit quantization of the 753B-parameter model fits a single 24 GB GPU.
...
- **Blurb:** Baidu published Unlimited-OCR, an MIT-licensed document-parsing model framed as an advance on DeepSeek-OCR with multi-page support.
...
- **Blurb:** Researchers report Moebius, a 226M-parameter image-inpainting model claiming parity with 11.9B-parameter FLUX.1-Fill-Dev at 15x runtime speedup.
...
- **Blurb:** VibeThinker-3B, a 3B reasoning model trained with 'Spectrum-to-Signal', reportedly matches much larger math and coding models.
...
- **Blurb:** Claude Code's Extended Thinking display is a summary of reasoning, not raw reasoning itself, a behavior Anthropic's documentation already states.
...
- **Blurb:** Spur scanned 6,038 TV apps and flagged 2,058 LG and Samsung models bundling residential-proxy SDKs, creating a supply-chain home-network pivot.
...
- **Blurb:** CISA Known Exploited Vulnerabilities catalog remains at 1,623 entries with no new active-exploitation entry since Splunk's recent addition.
...
- **Blurb:** Oak, a VCS alternative for agents, uses content-defined chunking and lazy hydration to start editing large repos in seconds.
...
- **Blurb:** Homebrew 6.0.3 refines tap-trust behavior by allowing trust inside the build sandbox and removing warnings for partially trusted skipped taps.
...
- **Blurb:** Rhombus reached v1.0, a functional language on Racket with conventional syntax and pattern matching, relating to Racket as Elixir does to Erlang.
...
- **Blurb:** Crunchy Data shows why Postgres timestamptz with IANA zones prevents date-math bugs, using BC's daylight-saving suspension as a worked example.
...
- **Blurb:** A post advocates memcached over Redis for pure caching: failures return defaults and no persistence keeps the cache ephemeral.
...
- **Blurb:** Practical Programming, Fourth Edition updates a Python CS introductory text to 3.14 with f-strings and type annotations, releasing July 2026.
...
- **Blurb:** AI data-center demand for DRAM and NAND tightens supply so far that retro memory formats are rising in price, extending the semiconductor crunch.
...
- **Blurb:** SpaceX fell 16.4% to $154.60 on 2026-06-22; the all-stock Anysphere acquisition is priced on SPCX's seven-day VWAP, lowering the deal value.
...
- **Blurb:** A GLM-5.2 versus Claude Opus 4.8 comparison on WebGL drew 480 HN points; commenters noted single prompts are not representative benchmarks.
...
- **Blurb:** A benchmark found Mythos unique on four bugs but argues cheaper models like Qwen and DeepSeek suggest the edge is economic, not capability.
...
- **Blurb:** Deno Desktop remained over 1,000 HN points; discussion centered on compile-time permission inheritance and the Electron-versus-native-UX debate.
...
- **Blurb:** MLST interviewed John Jumper about his move from DeepMind to Anthropic, providing context to an AI-for-science talent shift by Nobel laureate.
...
- **Blurb:** Reddit's r/programming surfaced practitioner discussion on epoll versus io_uring, Project Valhalla for JDK 28, and Apple devicectl tools.
