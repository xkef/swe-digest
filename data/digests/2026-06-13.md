...
- **Blurb:** Anthropic disabled Fable 5 and Mythos 5 for all users after US export directive citing a narrow jailbreak vulnerability.
...
- **Blurb:** CVE-2026-35273, an unauthenticated RCE in Oracle PeopleSoft, was exploited for two weeks before patching and added to CISA KEV.
...
- **Blurb:** DepthFirst's autonomous agent discovered 21 FFmpeg vulnerabilities for about $1,000, including an RCE in the AV1 RTP depacketizer.
...
- **Blurb:** Claude Sonnet 4 and Opus 4 API access ends 2026-06-15 at 09:00 PT with no grace period; migrate to newer versions.
...
- **Blurb:** Linux 7.1 release candidate 7 was the expected final candidate; Linus Torvalds plans stable release on 2026-06-14.
...
- **Blurb:** Moonshot AI released Kimi K2.7-Code, an open-weight agentic coding model with 1 trillion parameters and reduced thinking-token usage.
...
- **Blurb:** A widely discussed guide covers running local coding agents on macOS with llama.cpp and ollama, with mixed speedup results on Apple Silicon.
...
- **Blurb:** Attackers hijacked over 1,500 AUR packages and injected malicious npm dependencies delivering an infostealer with an optional eBPF rootkit.
...
- **Blurb:** A researcher reported an RCE in AMD's auto-updater exploitable via MITM due to CRC32 validation; AMD initially denied the bounty.
...
- **Blurb:** Facebook, Instagram, WhatsApp, and Messenger experienced a four-hour global outage affecting login and feed access on 2026-06-12.
...
- **Blurb:** Cloudflare's Dashboard and API control plane were unavailable on 2026-06-12; CDN and security features continued serving unaffected.
...
- **Blurb:** tmux 3.7 release candidate adds floating panes as a new structural layout feature, plus enhancements to copy mode and shell expansion.
...
- **Blurb:** Homebrew 6.0.1 addresses tap and bundle handling regressions introduced in the 6.0.0 major release from 2026-06-11.
...
- **Blurb:** Spring Boot 4.1.0 reaches GA with first-class gRPC support, expanded OpenTelemetry configuration, and SSRF mitigation via InetAddressFilter.
...
- **Blurb:** WASI 0.3.0 ratified with async moved into the WebAssembly Component Model canonical ABI, replacing WASI 0.2 pollable patterns.
...
- **Blurb:** Apple rewrote the TrueType hinting interpreter from C to memory-safe Swift, achieving 13 percent faster performance in font handling.
...
- **Blurb:** PostgreSQL 19 Beta 1 adds application-time temporal table support, parallel autovacuum, SQL graph queries, and disables JIT by default.
...
- **Blurb:** A practitioner guide describes design constraints, component reuse, and review checkpoints to reduce low-quality AI-generated front-end code.
...
- **Blurb:** SpaceX began trading at $135 per share after a $75 billion raise and became eligible for early MSCI inclusion on 2026-06-13.
...
- **Blurb:** A 1,554-point HN essay argues low-effort AI-generated requests should be deprioritized, echoing maintainer burden concerns from open source.
...
- **Blurb:** An opinion piece arguing for open-weight AI drew 661 points; commenters debated whether open weights protect against government access controls.
