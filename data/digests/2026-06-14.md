...
- **Blurb:** Amazon CEO Andy Jassy's discussions with Trump officials preceded a US directive suspending Anthropic's Fable 5 and Mythos 5 models globally.
...
- **Blurb:** Linus Torvalds released Linux 7.1-rc7 as the expected final candidate, with stable launch anticipated 2026-06-14 and FRED and NTFS support.
...
- **Blurb:** Claude-sonnet-4-20250514 and claude-opus-4-20250514 retire from the Claude API 2026-06-15 at 09:00 PT with immediate request failure.
...
- **Blurb:** CVE-2026-35273, unauthenticated RCE in Oracle PeopleSoft, was exploited as a zero-day by ShinyHunters from 2026-05-27 to 2026-06-09.
...
- **Blurb:** Zhipu announced GLM 5.2 with 1M token context and open-weight release expected the following week on its API and Coding Plan.
...
- **Blurb:** Anthropic reports Opus 4.7 reaching average errors comparable to ChemDraw and MestReNova on nuclear magnetic resonance spectral analysis.
...
- **Blurb:** Practitioners report running coding agents on cheap-provider APIs and self-hosted models costs roughly $10 over two weeks.
...
- **Blurb:** Arch AUR supply-chain attack modified over 1,500 packages' PKGBUILD files to fetch malware with infostealer and eBPF rootkit capability.
...
- **Blurb:** A Honda Civic head unit verifies USB updates against the public AOSP test key, allowing arbitrary code installation with physical access.
...
- **Blurb:** Pyodide 314.0 and PEP 783 let maintainers publish WebAssembly wheels to PyPI like native wheels, removing per-project custom build steps.
...
- **Blurb:** Nikita Prokopov's post covers rendering smooth UI animation by managing frame pacing between display refresh and render timing.
...
- **Blurb:** Google Research describes repurposing retired smartphones as distributed low-carbon compute platform, reusing processors and batteries.
...
- **Blurb:** Arabic text rendering requires contextual letterforms, kashida justification, bidirectional algorithm, and complex font architecture support.
...
- **Blurb:** State attorneys general are investigating OpenAI's governance and corporate structure during the company's confidential S-1 filing process.
...
- **Blurb:** Differential-privacy researcher Damien Desfontaines argues against a legislative provision banning noise-infusion from Census Bureau statistics.
...
- **Blurb:** LLM-tooling startup TensorZero archived its repository one year after $7.3M seed, prompting venture-backed open-source sustainability concern.
