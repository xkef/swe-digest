...
- **Blurb:** Open-weight GLM 5.2 scored 39 percent F1 on IDOR detection versus Claude Code's 32 percent, at 0.17 USD per vulnerability found.
...
- **Blurb:** Codex lacks a .codexignore mechanism to exclude secrets and credentials from being read and sent to the model, open since 2025-08-28.
...
- **Blurb:** A developer used Claude Code to cross-reference an MRI report against clinical guidelines to question a treatment recommendation.
...
- **Blurb:** Librepods, a GPL-3.0 project, reverse-engineered AirPods control features to expose battery, noise control, and gestures on Android and Linux.
...
- **Blurb:** A preprint argues that evaluating coding agents one at a time misses ecosystem harm when many agents commit to shared repositories concurrently.
...
- **Blurb:** 2,150-case benchmark found tool-using agents completed tasks while inappropriately disclosing private data through intermediate tool calls.
...
- **Blurb:** An opinion post argues that early agent tokenmaxxing is replaced by compounding correctness, where more tokens tend to yield better outcomes.
...
- **Blurb:** Erik Schamper reverse-engineered ASIF, Apple's sparse virtual-disk format for macOS virtual machines, and released an open-source reader.
...
- **Blurb:** LineShine, running custom domestic processors and interconnect, became the first Chinese system to lead TOP500 since Sunway TaihuLight in 2017.
...
- **Blurb:** Cloudflare traced a six-week image-truncation bug to a discarded Poll::Pending in hyper's HTTP/1 state machine, a four-line fix in dispatch.rs.
...
- **Blurb:** One resume scored 66 to 99 out of 120 through HackerRank's open-source resume-scoring agent, revealing unreliable evaluative judgment.
...
- **Blurb:** A class action alleges Samsung, SK Hynix, and Micron coordinated DRAM pricing since 2022, with prices rising approximately 700 percent.
...
- **Blurb:** An opinion essay arguing mandatory age verification precedes persistent identity attribution of online speech topped the front page.
...
- **Blurb:** A Stanford data page plotting per-byte prices for DRAM, flash, and disk memory from 1960 to 2026 reached the front page with 167 points.
...
- **Blurb:** Hacker News commenters scrutinized the GLM 5.2 benchmark, arguing it pits a bare prompt against Semgrep's scaffolded multi-agent pipeline.
