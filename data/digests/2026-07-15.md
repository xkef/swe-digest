...
status = "published"
source_count = 31
lede = "Security flaws disclosed in Cursor, Claude.ai, Microsoft, and Tailscale, including unpatched code execution and memory exfiltration."
+++
...
- **Blurb:** Mindgard disclosed an unpatched Cursor flaw on Windows where a malicious git.exe in a repository root runs automatically with no prompt.
...
- **Blurb:** Ayush Paul showed Claude.ai memory exfiltration through web_fetch via a fake CAPTCHA with alphabetical links spelling out user data.
...
- **Blurb:** GitHub added a default package cooldown to Dependabot version updates, delaying version-update pull requests but not security patches.
...
- **Blurb:** PrismML published Bonsai 27B, shrinking Qwen 3.6 from 54 GB to 3.8 GB through extreme quantization to ternary for on-device inference.
...
- **Blurb:** EuroPython 2026 runs from 2026-07-13 through 2026-07-19, covering CPython internals, typing, packaging, and the scientific Python stack.
...
- **Blurb:** Colin Snyder published Star Fleet, running parallel Codex agents to emit machine-checked Lean 4 proofs for open Erdős problems.
...
- **Blurb:** Juggler is an open-source coding agent replacing linear chat with branching Yjs CRDT documents navigated through Miller-column UI.
...
- **Blurb:** An essay on martinfowler.com argues domain-specific languages improve LLM reliability by constraining output and enabling self-correction.
...
- **Blurb:** Microsoft's July 2026 Patch Tuesday addressed a record 569 CVEs, including two actively exploited zero-days in ADFS and SharePoint Server.
...
- **Blurb:** CISA added CVE-2026-56164, an authentication-bypass in SharePoint Server, to KEV with a three-day federal remediation deadline.
...
- **Blurb:** Tailscale SSH argument-injection via leading-dash usernames allowed getent to output the passwd file to authorized tailnet principals.
...
- **Blurb:** OpenAI reported short ChatGPT incidents on 2026-07-14 and 2026-07-15 affecting voice mode and conversation features, with the API unaffected.
...
- **Blurb:** Cloudflare R2 continued elevated errors in Western Europe through 2026-07-15 during scheduled maintenance in its Barcelona datacenter.
...
- **Blurb:** Homebrew 6.0.11 adds a `brew vulns` command and OSV advisory export, moving dependency scanning closer to the package install path.
...
- **Blurb:** Marco Nett measured X11 and native Wayland input latency both at 4 milliseconds, showing driver and compositor matter more than protocol.
...
- **Blurb:** S&P cut Oracle to BBB- on AI datacenter debt, projecting fiscal 2027 capex of 90 to 95 billion dollars with OpenAI accounting for half.
...
- **Blurb:** Johanna Larsson described a Claude Code hook that rewrites recurring phrases like load-bearing and stock responses before display.
...
- **Blurb:** Armin Ronacher argues agents remove coordination friction but let isolated changes accumulate into incoherent systems over time.
