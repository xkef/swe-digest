...
- **Blurb:** Anthropic released Claude Sonnet 5 same-day with introductory pricing through August, improving coding and agentic capabilities over Sonnet 4.6.
...
- **Blurb:** CISA added CVE-2026-48558 to Known Exploited Vulnerabilities; SimpleHelp OIDC bypass enables unauthenticated technician access on servers.
...
- **Blurb:** A practitioner argued Alibaba's Qwen3.6-27B is the first general-purpose local model, running at 32 tokens per second on high-end Apple Silicon.
...
- **Blurb:** Samsung and SK Hynix committed record capex to a Korea chip cluster amid intensifying AI memory demand and supply competition with China.
...
- **Blurb:** HCCF proposed .self top-level domain offering one free domain per verified person for self-hosting with simplified DNS and shared infrastructure.
...
- **Blurb:** Anthropic released Claude Science beta, a desktop research workbench for life sciences with native visualization and database integrations.
...
- **Blurb:** Google released Nano Banana 2 Lite, a distilled Gemini image model for fast, cost-effective generation at scale in high-volume production.
...
- **Blurb:** Meituan announced LongCat-2.0, a 1.6-trillion-parameter MoE with 1M-token context and MIT license, reportedly trained on non-NVIDIA ASICs.
...
- **Blurb:** deepreinforce-ai released Ornith-1.0, open-weight agentic-coding models using reinforcement learning from 9B to 397B parameters.
...
- **Blurb:** A preprint cataloged MCP server architecture patterns and anti-patterns from fifteen servers, reporting tool-selection accuracy thresholds.
...
- **Blurb:** Git 2.55.0 implemented a Linux fsmonitor daemon for faster status checks and disabled remote terminal control sequences by default.
...
- **Blurb:** ZLUDA Version 6 added pre-alpha PhysX support and texture functionality to run unmodified CUDA applications on AMD GPUs through ROCm.
...
- **Blurb:** Microsoft put WSL Container into public preview, enabling Linux container management in Windows Subsystem for Linux without Docker Desktop.
...
- **Blurb:** Outer Shell prototypes a graphical front end for SSH administration using native SwiftUI and automatic binary download, macOS only.
...
- **Blurb:** Fil-C documented memory-safe setjmp/longjmp and ucontext implementations with jump validation and garbage-collection integration.
...
- **Blurb:** Spencer Bryngelson reverse-engineered the Apple Neural Engine, documenting the fixed-function matrix accelerator across A11-A18 and M1-M5.
...
- **Blurb:** Fergus Finn traced a CUDA kernel from nvcc to GPU across every layer, reverse-engineering closed-source libraries to expose hardware behavior.
...
- **Blurb:** Seth Humphries described WATaBoy, a Game Boy emulator JIT-compiling to WebAssembly bytecode, running 1.2x faster than a native interpreter.
...
- **Blurb:** An essay arguing age verification precedes speech attribution reached HN's front page, reflecting identity-gate debates in AI services.
...
- **Blurb:** A blog post claimed Claude Code embeds invisible Unicode in requests as steganographic fingerprint for distillation detection.
...
- **Blurb:** A Tell HN post reported Cursor iOS alters device privacy settings irreversibly, unconfirmed by the vendor or independently.
...
- **Blurb:** Waag criticized EU digital-identity wallets for depending on Google and Apple device attestation, excluding de-Googled OSes.
