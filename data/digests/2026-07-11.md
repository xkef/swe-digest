...
- **Blurb:** Apple sued OpenAI and two former employees for allegedly stealing confidential documents and directing recruits to bring hardware to interviews.
...
- **Blurb:** OpenAI's GPT-5.6 Sol Ultra produced an unverified proof of the Cycle Double Cover Conjecture, a 50-year-old mathematics problem.
...
- **Blurb:** JetBrains disclosed CVE-2026-59793, a CVSS 8.8 arbitrary file-access flaw in TeamCity's Perforce integration, fixed in version 2026.1.2.
...
- **Blurb:** ICML 2026 concludes on its final day, with papers and session recordings becoming available through the conference program site.
...
- **Blurb:** EuroPython 2026 starts in two days and runs through July 19 in Prague, setting the agenda for CPython, packaging, and typing discussions.
...
- **Blurb:** Three repositories packaging reusable skills for AI coding agents trended on GitHub, consolidating a shared distribution format for workflows.
...
- **Blurb:** Coinspect disclosed Ill Bloom, a weak-RNG flaw in wallet software that drained millions from crypto addresses across multiple blockchains.
...
- **Blurb:** LaunchDarkly suffered an outage affecting web access and flag delivery, requiring server-side SDK restarts to restore connections.
...
- **Blurb:** crates.io was hit by a coordinated denial-of-service attack against API servers; package downloads and dependency resolution were unaffected.
...
- **Blurb:** Prometheus 3.13.1 fixes a TSDB bug in which the head-chunk cache could return incorrect samples or spurious not-found errors on range queries.
...
- **Blurb:** Scarf moved production off Haskell to Python after seven years, citing slow builds and friction with LLM-assisted parallel development.
...
- **Blurb:** LWN details how AI scrapers route through residential proxies to bypass IP blocking, raising operational costs for public infrastructure.
...
- **Blurb:** ClickHouse scales single-threaded PgBouncer across cores using SO_REUSEPORT to load-balance connections, achieving roughly 4x total throughput.
...
- **Blurb:** A cosmetic quicksort rewrite triggered branchless codegen, cutting sort time from 4.39 to 0.70 seconds, roughly 6.3x faster than before.
...
- **Blurb:** Ghostty creator Mitchell Hashimoto demonstrated scrollback compression, a feature reducing memory overhead for large terminal buffers.
...
- **Blurb:** Colibri runs a 744-billion-parameter mixture-of-experts model on a laptop by streaming routed experts from disk through an LRU cache.
...
- **Blurb:** Cursor users reported Grok 4.5 missing from the model picker; SpaceX states the model is not initially available in the EU until mid-July.
