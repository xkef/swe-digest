...
- **Blurb:** OpenAI's 2025 financials, verified by the Financial Times, show 13.07B USD revenue against 34B USD costs, yielding a 38.53B USD net loss.
...
- **Blurb:** Z.ai published GLM-5.2, a 753B-parameter open-weight model under MIT, runnable locally and removing China-hosted-API data handling concerns.
...
- **Blurb:** Tesco is migrating 40,000 workloads off VMware by end of 2027 following Broadcom's licensing changes and disputed perpetual-license refusal.
...
- **Blurb:** GPT-5.4 improved the difficult Chan-Lam coupling with primary sulfonamides, raising yields for most tested substrates in wet-lab experiments.
...
- **Blurb:** Cloudflare opened Agents SDK durable-execution primitives to outside frameworks, with Flue as the first supported TypeScript agent framework.
...
- **Blurb:** Ellis documented local Qwen as different from Opus: strong on diagnostics and codebase work, weak on long-horizon unsupervised concurrent work.
...
- **Blurb:** Three FortiSandbox vulnerabilities including a CVSS 9.8 path-traversal flaw are under active exploitation despite April 2026 security patches.
...
- **Blurb:** Researcher found 10,000 GitHub repositories using legitimate code but distributing trojanized archives through README links as delivery vector.
...
- **Blurb:** Node.js released security patches for v26.3.1, v24.17.0, and v22.23.0 on June 18, fixing eleven CVEs including two High-severity TLS flaws.
...
- **Blurb:** AMD disabled TSME memory encryption on consumer Ryzen without a visible BIOS change, removing protection against physical cold-boot attacks.
...
- **Blurb:** OpenAI reported ChatGPT load failures and Enterprise SSO errors on June 18, with mitigation applied for SSO issues but no root cause disclosed.
...
- **Status:** confirmed
- **Sources:** [OpenAI status history](https://status.openai.com/history)
- **Blurb:** OpenAI logged Android and iOS app errors plus FedRAMP workspace performance degradation on June 17, with no published root cause analysis.
...
- **Blurb:** Otty, a closed-source paid macOS terminal, offers GPU acceleration and first-class support for coding agents running in the terminal.
...
- **Blurb:** Emacs 31.0.90 enables tree-sitter by default, adds auto-grammar installation for supported modes, and provides editable grep-style xref workflow.
...
- **Blurb:** Kerkour argues stdx avoids crates.io via Git-only distribution to reduce supply-chain risk and prevent namespace squatting of the stdlib project.
...
- **Blurb:** Simmons reported NetNewsWire modernization with 2,188 commits, Swift concurrency migration, Liquid Glass UI, and performance work.
...
- **Blurb:** Prometheus 3.5.4 LTS patches plaintext secret exposure in STACKIT service discovery's /-/config endpoint and updates golang and OpenTelemetry.
...
- **Blurb:** Browser Use used Firecracker snapshots and page-size tuning to cut microVM browser starts from 9.8s to 3.1s and creation costs by 67 percent.
...
- **Blurb:** DOJ moved to shield xAI's unpermitted gas turbines citing national, economic, and energy security concerns over potential Clean Air Act shutdown.
...
- **Blurb:** OpenRouter ran eleven models through 30 battle-royale games with tool use, reporting Grok won 13 matches at lower cost per win vs Claude Sonnet.
...
- **Blurb:** Midjourney Medical proposed AI ultrasound screening, but radiologists questioned false-positive rates and image quality vs standard ultrasound.
