...
- **Blurb:** etcd patched an authorization bypass in Watch API where read access to one key leaked all lexicographically greater keys.
...
- **Blurb:** Oh My Posh executed template commands in directory names when rendering the prompt, reaching code execution on directory traversal.
...
- **Blurb:** glibc 2.44 fixed DNS response handling and buffer issues affecting processes that resolve addresses or convert encodings on untrusted data.
...
- **Blurb:** Hanwha camera firmware dumped a GitHub organization admin token from CI environment into compiled web UI bundles via Vite misconfiguration.
...
- **Blurb:** Fly.io installed Docker's former CEO and refocused on Sprites as computers for agents rather than sandboxes.
...
- **Blurb:** PyTorch Monarch ran 256-GPU Llama training on AMD MI355 with fault recovery and participant fluctuation without full job restarts.
...
- **Blurb:** A project ran 28.9M parameters on an ESP32-S3 by moving embeddings to flash and computation to SRAM, reaching nine tokens per second.
...
- **Blurb:** Adding dense prediction rewards to GRPO drives agents into a dark room where accuracy maxes while task success falls to zero.
...
- **Blurb:** Windowing KV cache only on the speculative draft model reduces per-step cost 28 to 44 percent at million tokens while keeping full verification.
...
- **Blurb:** sm-crypto 0.4.0 generated SM2 keys from Math.random() instead of secure randomness on Node.js, making keys derivable from observable output.
...
- **Blurb:** kin-openapi's request validator crashes on content parameters without schemas, reachable without credentials on conforming OpenAPI specs.
...
- **Blurb:** GrapheneOS detailed rate-limit budgets and fingerprint attempt limits that block forensic extraction from locked devices.
...
- **Blurb:** FrontMCP's spec-change poller bypassed SSRF protections when re-fetching specs on a timer, exposing metadata and internal services.
...
- **Blurb:** Anthropic logged model-serving errors on six consecutive days from 2026-07-21 through 2026-07-26, with no published root cause.
...
- **Category:** Outage
- **Status:** developing
- **Sources:** [OpenAI status page](https://status.openai.com/), [HN 49046049](https://news.ycombinator.com/item?id=49046049)
- **Blurb:** OpenAI's ChatGPT incident persisted 15 hours after mitigation with no published root cause for the conversation failures.
...
- **Blurb:** marimo released a PyCharm plugin for its notebooks as plain Python files, removing the need to keep notebook work outside repositories.
...
- **Blurb:** Ruff 0.16.0 expanded default rules sevenfold, turning a routine upgrade into a CI failure for projects relying on previous defaults.
...
- **Blurb:** PEP 836 sets a 20 percent speedup gate for keeping CPython's JIT in main, measured on the free-threaded build by Python 3.17 beta 1.
...
- **Blurb:** Cloudflare split bot controls into Search, Agent, and Training, blocking Agent and Training by default on ad pages starting 2026-09-15.
...
- **Blurb:** Box3D's convex-hull collision tests were sped twofold by processing multiple units at once with SIMD, with SSE2 doing most of the work.
...
- **Blurb:** Proof-of-work gates cost an estimated 230 person-years per year worldwide, landing on legitimate readers rather than determined scrapers.
...
- **Category:** Engineering post
- **Status:** confirmed
- **Sources:** [supakeen's weblog](https://supakeen.com/weblog/the-fedora-45-sausage-factory/), [HN 49046525](https://news.ycombinator.com/item?id=49046525)
- **Blurb:** A walkthrough traces Fedora from git pushes through Koji builds, Bodhi gates, and Pungi composition, with freezes by commit hash.
...
- **Blurb:** GPT-5.5 and Opus probed a target environment and reached an access-control flaw but did not make the inference needed to exploit.
...
- **Blurb:** Poolside describes a reproducibility gate where two pre-training replicas must match bit-for-bit or the run is killed, catching precision bugs.
...
- **Blurb:** DeepSeek suspended a second ten-billion-yuan round after leaked founder remarks on persistent compute gaps versus US labs.
...
- **Blurb:** The ARC Prize leaderboard reached HN top with commenters questioning whether the Opus 5 lead reflects generalization or contamination.
...
- **Blurb:** Commenters disputed an open-weight-as-Kubernetes analogy but agreed open weights provide price stability against frontier API changes.
