...
- **Blurb:** European Parliament voted to extend voluntary message scanning on non-encrypted services until 2028 after narrowly failing to reject it.
...
- **Blurb:** Meta released Muse Spark 1.1, a multimodal agentic model with 1 million token context and API pricing that undercuts Grok 4.5.
...
- **Blurb:** Tencent released Hy3, an Apache 2.0 open-weight mixture-of-experts model with 295B parameters and 21B active per token.
...
- **Blurb:** A Rust reimplementation of PostgreSQL passes the full regression suite and boots from existing data directories but is not production-ready.
...
- **Blurb:** The main annual Python community conference starts 2026-07-13 and covers language, packaging, typing, performance, and ecosystem updates.
...
- **Blurb:** ICML runs through 2026-07-11 with conferences and workshops on training methods, reasoning, evaluation, and interpretability research.
...
- **Blurb:** Mitchell Hashimoto discussed the design of Ghostty terminal emulator and his experience building it in Zig, including language tradeoffs.
...
- **Blurb:** The first Linux device trees for Apple M4 reach bootable state but require further work on peripherals and stable SMP boot.
...
- **Blurb:** The International Earth Rotation Service confirmed no leap second will be added at the end of 2026, keeping the UTC offset unchanged.
...
- **Blurb:** Prometheus 3.13.1 fixes a TSDB bug where the head-chunk cache returned incorrect or missing results on range queries in the LTS line.
...
- **Blurb:** Ginger Bill argues that well-designed tools disappear from focus and prioritize sensible defaults over maximal configurability.
...
- **Blurb:** Code quality matters more, not less, when LLMs write it, because poor code trains models to repeat shortcuts in future requests.
...
- **Blurb:** A project streams large model weights from SSD to run GLM 5.2 on memory-constrained machines, trading throughput for lower memory usage.
...
- **Blurb:** HN debated whether databases earn reliability from production exposure rather than passing test suites and questioned AGPL licensing.
